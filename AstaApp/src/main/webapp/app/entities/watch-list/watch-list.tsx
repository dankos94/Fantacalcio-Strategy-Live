...
import { getEntities } from './watch-list.reducer';
...
export const WatchList = () => {
  ...
  const watchListList = useAppSelector(state => state.watchList.entities);
  ...
  return (
    <div>
      <h2 id="watch-list-heading" data-cy="WatchListHeading">
        WatchLists
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} /> Refresh list
          </Button>
          <Link to="/watch-list/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp; Genera un nuovo Watch List
          </Link>
        </div>
      </h2>
      <div className="table-responsive">
        {watchListList && watchListList.length > 0 ? (
            ...
            <tbody>
              {watchListList.map((watchList, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/watch-list/${watchList.id}`} color="link" size="sm">
                      {watchList.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button tag={Link} to={`/watch-list/${watchList.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
                        <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Modifica</span>
                      </Button>
                      <Button
                        onClick={() => (window.location.href = `/watch-list/${watchList.id}/delete`)}
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No Watch Lists found</div>
        )}
      </div>
    </div>
  );
